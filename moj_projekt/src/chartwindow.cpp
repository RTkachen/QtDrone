 ...
 */
void ChartWindow::openChartWindow(QObject *serialHandler)
{
    ...
    if (!instance || !instance->isVisible()) {
        ...
        // Zakładamy, że serialHandler ma sygnał newData(double, double, double)
...
void ChartWindow::setupChart(int index, const QString &title)
{
    ...
    axisY[index]->setTitleText(tr("Value"));
...
void ChartWindow::updateTranslations() {
    // Aktualizujemy statyczne elementy interfejsu:
    ui->retranslateUi(this);

    // Aktualizujemy etykiety osi i tytuły wykresów (jeśli są ustawiane dynamicznie)
    for (int i = 0; i < 3; ++i) {
        if (axisX[i]) {
            axisX[i]->setTitleText(tr("Time (s)"));
        }
        if (axisY[i]) {
            axisY[i]->setTitleText(tr("Value"));
        }
        // Jeżeli tytuł wykresu jest ustawiany dynamicznie:
        if(chartViews[i]) {
            QChart *chart = chartViews[i]->chart();
            if (chart) {
                // Można ewentualnie ustawić tytuł wykresu na nowo, np.:
                chart->setTitle(tr("Chart %1").arg(i+1));
            }
        }
    }
}
